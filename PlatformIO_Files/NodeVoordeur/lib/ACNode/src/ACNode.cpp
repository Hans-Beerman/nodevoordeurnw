...
void ACNode::set_device1(const char *p)  { strncpy(device1,p, sizeof(device1)); };
void ACNode::set_device2(const char *p)  { strncpy(device2,p, sizeof(device2)); };
...
ACNode::ACNode(const char * m, bool wired, const char * dev1, const char * dev2, acnode_proto_t proto) : 
	_ssid(NULL), _ssid_passwd(NULL), _wired(wired), _proto(proto)
{
    ...
    if (dev1 && *dev1) {
      strncpy(device1, dev1, sizeof(device1));
    }
    if (dev2 && *dev2) {
      strncpy(device2, dev2, sizeof(device2));
    }
    pop();
}

ACNode::ACNode(const char *m, const char * ssid , const char * ssid_passwd, const char * dev1, const char * dev2, acnode_proto_t proto ) :
...
void ACNode::request_approval_devices(const char * tag, const char * operation, const char * target, bool useCacheOk) {
    if (device1 && *device1) {
        request_approval(tag, operation, device1, useCacheOk);
    }
    if (device2 && *device2) {
        request_approval(tag, operation, device2, useCacheOk);
    }
    if (!(device1 && *device1) && !(device2 && *device2)) {
        request_approval(tag, operation, target, useCacheOk);
    }
} 
...
//		return;
	...
	Debug.printf("Requesting approval for %s at node %s on machine %s by tag %s\n", 
		operation ? operation : "<null>", moi ? moi: "<null>", target ? target : "<null>", tag ? "*****" : "<null>");
